(()=>{
  ...
  function uploadResultText(report){
    const s=report?.parser_status||'pending';
    if(s==='ready_for_review'||s==='normalized')return 'Report processed successfully. Your credit data is ready for review below.';
    if(s==='needs_ocr')return 'Report uploaded, but this PDF needs OCR review before CreditOS can read it.';
    if(s==='needs_review')return report?.error_message||'Report uploaded, but parser review is required.';
    if(s==='failed')return report?.error_message||'The report uploaded but processing failed.';
    return 'Report uploaded successfully. CreditOS is processing the report now.';
  }
  ...
  form?.addEventListener('submit',async e=>{
    ...
    const button=form.querySelector('button[type=submit]');button.disabled=true;button.textContent='Processing…';message.className='import-message';message.textContent='Uploading securely, extracting report text, and preparing your CreditOS review…';
    try{const data=new FormData(form);const result=await api('reports/import',{method:'POST',body:data});if(!result)return;const bad=['needs_ocr','needs_review','failed'].includes(result.report?.parser_status);message.className=bad?'import-message error':'import-message success';message.textContent=uploadResultText(result.report);form.reset();fileName.textContent='No file selected';await loadReports(result.report_id);}
