...
class CreditOS_Report_Import {
    ...
    private function harden_private_report_file($attachment_id){
        ...
        return false!==$perms&&0600===($perms&0777);
    }
    ...
    public function register_routes(){
        ...
        register_rest_route('creditos/v1','/reports/(?P<id>\d+)/tradelines/(?P<tradeline_id>\d+)/corrections',array('methods'=>WP_REST_Server::CREATABLE,'callback'=>array($this,'create_tradeline_correction'),'permission_callback'=>array($this,'client_access')));
    ...
    public function import_report(WP_REST_Request $request){
        ...
        if(!$rid){wp_delete_attachment($aid,true);return new WP_Error('creditos_report_import_failed','CreditOS could not create the report record. The uploaded file was removed.',array('status'=>500));}
    ...
    public function compare_report_versions(WP_REST_Request $request){
        ...
        $snap=array(); foreach($rows as $row){$stored=(string)($row['snapshot_hash']??'');if($stored!==''&&!hash_equals($stored,hash('sha256',(string)$row['snapshot']))){$this->repository->audit(get_current_user_id(),$client->id,'credit_report_version_integrity_failed','credit_report',$rid,array('version_number'=>(int)$row['version_number'],'reason'=>'hash_mismatch'));return new WP_Error('creditos_version_integrity_failed','A report version failed its integrity check and cannot be compared.',array('status'=>409));}$decoded=json_decode((string)$row['snapshot'],true);if(!is_array($decoded)||JSON_ERROR_NONE!==json_last_error()){$this->repository->audit(get_current_user_id(),$client->id,'credit_report_version_integrity_failed','credit_report',$rid,array('version_number'=>(int)$row['version_number'],'reason'=>'invalid_snapshot_json'));return new WP_Error('creditos_version_integrity_failed','A report version failed its integrity check and cannot be compared.',array('status'=>409));}$snap[(int)$row['version_number']]=$decoded;}
    ...
    public function list_saved_reviews(WP_REST_Request $request){
        $client=$this->current_client(); if(!$client)return new WP_Error('creditos_client_missing','CreditOS client profile could not be loaded.',array('status'=>404));
        $table=$this->wpdb->prefix.'creditos_tradelines'; $reports=$this->wpdb->prefix.'creditos_credit_reports';
        $status=sanitize_key($request->get_param('status')?:''); $allowed=array('verified','needs_review','potential_inaccuracy','missing_evidence','ignore');
        $where="t.client_id=%d AND (t.reviewed_at IS NOT NULL OR t.reviewer_notes<>'' OR t.discrepancy_type IS NOT NULL OR t.evidence_status<>'not_requested')";
        $args=array($client->id); if($status&&in_array($status,$allowed,true)){$where.=" AND t.review_status=%s";$args[]=$status;}
